...
# Sample the ROUGHEST ground, not the highest — whatever holds there holds
# everywhere flatter. Here that is the Veluwe, not Limburg: Limburg has 45x the
# relief and LESS pixel-scale roughness, so relief is the wrong proxy.

# PNG crops are resampled with cubic, not nearest. Nearest turns the coarser
# zooms into hard blocks, which reads as "much worse" for reasons unrelated to
# terrain detail, and has produced a wrong call once already.

# Run via:
#   nice ~/miniforge3/bin/conda run --no-capture-output -n geo nu ~/fm/freemap-outdoor-map/scripts/sample-zoom-nl.nu

# ── Configuration ─────────────────────────────────────────────────────────────

const SRC_VRT = "/run/media/martin/2190983A5767510F/AHN/dtm_05m/all.vrt"
const OUT_DIR = "/run/media/martin/18TB/nl/zoom-sample"
const NODATA  = "-9999"
const ZOOMS   = [15 16 17]                  # ascending; the last is the reference
# Veluwe near Posbank: the roughest low-nodata ground measured (0.0964 mean
# |Laplacian|, 3.1% nodata). Limburg has 45x the relief and less roughness, so
# relief is the wrong thing to sample here.
const WIN_X0  = 199000                      # RD New, window top-left
const WIN_Y1  = 449500
const WIN_M   = 1250                        # matches shading-nl.nu STEP
const COLLAR  = 6
const CROP    = 3
...
# For extrapolating each zoom's disk cost from this window.
const AREA_KM2 = 41540

# Latitude span, for converting a Web Mercator pixel size into real
...
t = Transformer.from_crs("EPSG:28992", "EPSG:3857", always_xy=True)
...
print $"==> Window: ($WIN_X0),($WIN_Y1) ($WIN_M) m"
...
let xmin = ($WIN_X0 | into float)
let ymax = ($WIN_Y1 | into float)
let xmax = ($xmin + $WIN_M)
let ymin = ($ymax - $WIN_M)
print $"    extent E ($xmin) .. ($xmax)   N ($ymin) .. ($ymax)  \(EPSG:28992\)"

# Read through the national VRT so the smoothing collar comes from neighbouring
# tiles. Costs nothing — GDAL opens only the tiles the window touches.
...
let projected = (
    $rendered | each {|r|
        let gb = ($r.bytes / $win_km2 * $AREA_KM2 / 1024 / 1024 / 1024)
        {
            zoom: $r.zoom
            ...
            gb_zstd: ($gb | math round -p 0)
            gb_jxl_est: ($gb / 3 | math round -p 0)
        }
    }
)
$projected | print
...
{
    window: $"($WIN_X0),($WIN_Y1)"
    window_m: $WIN_M
    extent_28992: {xmin: $xmin, ymin: $ymin, xmax: $xmax, ymax: $ymax}
...
print "    Set ZOOM in shading-nl.nu to whatever the numbers and the crops justify."
