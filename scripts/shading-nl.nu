...
# Shaded relief for the Netherlands from AHN5 (0.5 m). Port of shading-de-nw.nu.
...
# reads via wbgeotiff, which ignores the Predictor tag and decodes PREDICTOR=2/3
# float data as garbage without erroring.

# Grid origin is pinned below the AHN extent, not derived from it. Deriving it
# was the Belgium trap: adding a region moved every window id and a resumable
# run treated finished tiles as pending. Changing these renames every tile.
...
const STEP      = 1250                           # m; 2500 px at 0.5 m — see header
const COLLAR    = 6
const CROP      = 3

# Pinned below the AHN extent (X 10000.., Y 306250..) with room to spare.
const GRID_X0   = 0
const GRID_Y0   = 250000
...
if $_probe.exit_code != 0 or ($_probe.stdout | str trim | is-empty) {
    error make {msg: $"PROJ cannot resolve ($EPSG) — run via: nice ~/miniforge3/bin/conda run --no-capture-output -n geo nu shading-nl.nu"}
}
...
def render-window [w: record, tr: string, data_dir: string]: nothing -> nothing {
    ...
    # 3. 2 m DEM for contours-nl.nu — collar cropped, nodata-aware `average`.
