...
# Render one Thüringen window at several zoom levels so the ZOOM constant in
# shading-de-th.nu can be chosen from evidence instead of from analogy.
# Port of sample-zoom-en.nu; method deliberately unchanged.
#
...
# holds on the most detailed terrain in the country holds everywhere flatter.
# For Thüringen that is the Drachenschlucht at Eisenach, and it was CHOSEN BY
# MEASUREMENT rather than by reputation. Eight candidate sites were scored over
# a 1 km window on high-frequency residual after a 3x3 mean — what a hillshade
# actually renders — and the gorge won at 0.123 against 0.044-0.096 elsewhere.
#
# Relief alone would have picked differently and wrongly: Kyffhäuser (220 m of
# relief) and the Schwarzatal (208 m) both beat the Drachenschlucht's 160 m, yet
# both are smoother per pixel. The Großer Beerberg, the state's highest point,
# scores 0.055 — rounded ridge. If the slot gorge cannot justify a finer zoom,
# nothing in Thüringen can.
#
...
#
# Run via:
#   nice ~/miniforge3/bin/conda run --no-capture-output -n geo nu ~/fm/freemap-outdoor-map/scripts/sample-zoom-de-th.nu

# ── Configuration ─────────────────────────────────────────────────────────────

const SRC_DIR = "/run/media/martin/2190983A5767510F/DGM1/Thueringen"
const OUT_DIR = "/run/media/martin/18TB/de_th/zoom-sample"
const NODATA  = "-9999"
const ZOOMS   = [15 16 17]                  # ascending; the last is the reference
# Thuringian DGM1 tiles are 1 x 1 km, named dgm1_32_{E}_{N}_1_th_{vintage} where
# 32 is the UTM zone. This one holds the Drachenschlucht near Eisenach, measured
# the roughest ground in the state — see the header.
const TILE    = "dgm1_32_592_5646_1_th_2020-2025"
# 2 km, so FOUR tiles rather than one: these are 1 km tiles, and a 1 km window
# would be too small to judge. The VRT below spans the whole state, so the extra
# tiles and the collar cost nothing.
const WIN_M   = 2000
const COLLAR  = 6
const CROP    = 3
...
# Thuringia's land area, for extrapolating the disk cost of each zoom from the
# measured size of this window.
const ENGLAND_KM2 = 16202

# Latitude span of Thuringia, for converting a Web Mercator pixel size into real
# ground metres. Mercator pixels shrink as cos(lat), so the -tr passed to
# gdalwarp is NOT the ground resolution — at 50-52 degN it overstates it by
...
t = Transformer.from_crs("EPSG:25832", "EPSG:3857", always_xy=True)
...
let xmax = ($xmin + $WIN_M)
let ymin = ($ymax - $WIN_M)
print $"    extent E ($xmin) .. ($xmax)   N ($ymin) .. ($ymax)  \(EPSG:25832\)"

# VRT over ALL tiles, not just the chosen one. England's 5 km tiles left room to
# take the smoothing collar from inside a single tile; Thuringia's are 1 km and the
# window here IS the whole tile, so the collar has to come from the neighbours.
# Reading one small window through a 5000-tile VRT costs nothing — GDAL only
# opens the tiles the window actually touches.
ls $"($SRC_DIR)/*.tif" | get name | str join "\n" | save -f $"($d)/srclist.txt"
gdalbuildvrt -vrtnodata $NODATA -input_file_list $"($d)/srclist.txt" $"($d)/src.vrt" o> /dev/null
...
for r in $rendered {
    ...
    # All three crops are blown up to the same pixel size so they can be laid
    # side by side; with -r nearest the coarser zooms come out as hard blocks,
    # which reads as "much worse" for a reason that has nothing to do with how
    # much terrain detail they carry. That is not what a map client does when it
...

{
    tile: $TILE
    window_m: $WIN_M
    extent_25832: {xmin: $xmin, ymin: $ymin, xmax: $xmax, ymax: $ymax}
